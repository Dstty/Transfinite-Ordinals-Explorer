// ============================================================================
//  notation/LPMS.js — Lifting projection matrix system（LPMS / LPTSS）
// ============================================================================
//  移植自 ne-rewritten: src/notations/BM-like/LPMS.ts
//  表达式 = number[][]（BMS 矩阵）
//  id: lpms / lptss
// ============================================================================
;(() => {
  ...
  const lastAllZero = (matrix) =>
    matrix.length === 0 || matrix[matrix.length - 1].every((v) => v === 0);
    ...
    for (let h = 1; h <= n; h++) {
      for (let local = 0; local < B.length; local++) {
        ...
        const out = B[local].map(
          (v, r) => v + h * delta[r] * (r < t - 1 && ancestorContainsRoot(original, r + 1) ? 1 : 0),
        );
        result.push(out);
      }
    }
  ...
  const lpmsLimit = (expr) =>
    pseudoInfinity(expr) || (legal(expr) && !lastAllZero(standardize(expr, maxRows(expr))));

  const lpmsInfinityFs = (n) => {
    return [zeroCol(n), onesCol(n)];
  };

  const lptssInfinityFs = (n) => {
    return Array.from({ length: n + 1 }, (_, i) => [i, i, i]);
  };

  function make_lpms(id, name, infinityFs) {
    const variants = sequence_FS_variants0(lpmsFS, pseudoInfinity, infinityFs, lpmsLimit, BM_display);
    return {
      id,
      name,
      display: BM_display,
  ...
  register.push(make_lpms('lpms', 'Lifting projection matrix system', lpmsInfinityFs));
  register.push(make_lpms('lptss', 'Lifting projection triple sequence system', lptssInfinityFs));
})();
